...
public class StateDiff implements Persistable {
    ...
    public void read(byte[] bytes) {
        ...
        state = new HashMap<>();
        if(bytes != null) {
            for (i = 0; i < bytes.length; i += Hash.SIZE_IN_BYTES + 222) {
                state.put(new Hash(bytes, i, Hash.SIZE_IN_BYTES), new String(Arrays.copyOfRange(bytes, i + Hash.SIZE_IN_BYTES, i + Hash.SIZE_IN_BYTES + 222)));
